  ...
  <div>
    ...
    <ul>
      <li v-for="event in filtered">
        <h2 :id="event.name">
          <a :href="`#${event.name}`" aria-hidden="true" class="header-anchor"
            >#</a
          >
          {{ event.name }}
  ...
  computed: {
    filtered() {
      return this.events.filter(event =>
        this.query.length ? event.name.includes(this.query) : true
      );
    },
    events() {
      return documentation.events
        .map(event => ({
          ...event,
